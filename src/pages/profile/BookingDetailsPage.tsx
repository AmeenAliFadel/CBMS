...
export default function BookingDetailsPage() {
  ...
  const isValidBookingId =
    Number.isInteger(bookingIdValue) && bookingIdValue > 0;
  ...
  const booking =
    selectedBooking?.id === bookingIdValue ? selectedBooking : bookingFromList;
  ...
  const resolvedCarImage = booking?.car.images?.[0]?.url?.trim() || carImageSrc;
  ...
  const handleCancelBooking = async () => {
    ...
    if (booking.booking_plan === 1) {
      toast.error(
        "This booking uses the Basic plan. Cancellation is not available for this plan.",
      );
    ...
    <div className="min-h-screen bg-background px-3 py-4 sm:px-4 sm:py-6">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-5 sm:gap-6">
        ...
        <section className="grid grid-cols-1 gap-4 xl:grid-cols-2">
          ...
          >
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <BookingDetailsItem label="Booking ID" value={`#${booking.id}`} />
          ...
          >
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <BookingDetailsItem label="Car ID" value={`#${booking.car.id}`} />
          ...
          >
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <BookingDetailsItem label="Name" value={booking.customer.name} />
              <BookingDetailsItem
                label="Email"
        ...
        <BookingDetailsActionBar
          ...
          statusLabel={statusLabel}
          isBasicPlan={booking.booking_plan === 1}
        />
      </div>
    </div>
  );

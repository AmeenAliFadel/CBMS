...
import { bookingFormSchema, createBookingRequestSchema } from "../../../schemas/bookingSchemas";
...
export function useBookingPage(
    ...
    const pricing = useMemo(
        () => calculateBookingSummaryPricing(Number(car?.price_per_day ?? 0), bookingDays),
    ...
    const handleSubmit = useCallback(
        async (values: BookingFormValues) => {
            ...
            const requestValidation = createBookingRequestSchema.safeParse(
                requestPayload
            );
